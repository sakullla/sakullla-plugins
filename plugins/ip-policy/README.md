...
- 节点按当前插件的部署目标或自动策略的授权入口筛选，支持名称搜索和在线状态筛选。入口显示域名、监听端口或插件名称，内部 ID 和授权 token 不作为表单输入。
- 全局规则和省份白名单作用于所有使用本策略的入口；入口专属规则只补充到所选入口，拒绝结果优先。切换节点、查看入口或刷新状态会保留尚未保存的全局草稿。
- 地理数据从现有通用数据目录选择。导入文件时自动计算并验证 SHA-256，单文件最多 128 MiB，不启用定时下载、不另建插件文件缓存。分类和版本均从真实目录选择，校验或绑定失败保留原配置。共享来源仍受宿主的使用与回退引用保护，可显式删除不再使用的版本或来源。
- 卸载使用现有 `cleanup` 合同删除本插件实例、配置、授权和自有数据，并解除数据消费引用；共享数据不属于插件私有缓存，不误删其它消费者仍使用的数据。
- 诊断页只读取现有 `api/state` 返回的最近记录，在内存中筛选和分组展示，不新建主项目审计接口、采集器或持久事件库。当前宿主先取最近 100 条全局审计再筛选插件与节点，因此列表不代表完整历史；保留期限由宿主统一管理。

界面回归：`node plugins/ip-policy/testing/ui/run.mjs`。可通过 `NRE_UI_BROWSER` 指定 Chromium 浏览器；报告和三个宽度的截图输出到 `dist/ip-policy-ui/`。
