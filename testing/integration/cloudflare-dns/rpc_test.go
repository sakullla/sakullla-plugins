...
func runtimeFor(vault *fakeVault, dns *fakeDNS, trace *safeTrace) cloudflaredns.RuntimeAdapters {
	return cloudflaredns.RuntimeAdapters{Vault: vault, DNS: dns, Authorizer: cloudflaredns.AuthorizerFunc(func(context.Context, cloudflaredns.ActionContext) error { return nil }), UI: cloudflaredns.DynamicUIFunc(func(_ context.Context, record cloudflaredns.UIProjection) error { trace.addUI(record); return nil }), Auditor: cloudflaredns.AuditorFunc(func(_ context.Context, record cloudflaredns.AuditRecord) error { trace.addAudit(record); return nil }), Logger: cloudflaredns.EventLoggerFunc(func(_ context.Context, record cloudflaredns.EventRecord) error { trace.addLog(record); return nil })}
}
